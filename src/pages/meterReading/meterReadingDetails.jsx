...
const MeterReadingDetails = () => {
      ...
      try {
        ...
        try {
          const response = await axios.get(
            `${BASEURL}/meter-reading/normal/${id}`,
            {
          ...
          if (normalErr.response?.status === 404) {
            const response = await axios.get(
              `${BASEURL}/get-abnormal-reading/${id}`,
              {
                params: { tenantId: currentUser?.tenantId },
                withCredentials: true,
              },
        ...
        if (isPermissionDenied(err)) {
          ...
          setError(
            err.response?.data?.message ||
              "Failed to fetch meter reading details.",
          );
  ...
  const handleAnomalyInputChange = (field) => (e) => {
    setAnomalyDetails((prev) => ({
      ...prev,
      [field]:
        field === "reviewNotes" || field === "action"
          ? e.target.value
          : e.target.checked,
    }));
  };
  ...
  const validateReadingValues = () => {
    ...
    if (
      ...
      parseFloat(readingValues.reading) < 0
    ) {
  ...
  const handleUpdateAnomaly = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to update anomaly details.";
  ...
  const handleUpdateValues = async () => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setSnackbarMessage(
        validationErrors.reading || validationErrors.consumption,
      );
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
      return;
    ...
    try {
      ...
      const errorMessage =
        ...
        "Failed to update meter reading values.";
      setSnackbarMessage(errorMessage);
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
  ...
  if (error) {
    return (
      ...
      >
        ...
        <Alert
          severity="error"
          sx={{ mt: 2, borderRadius: 2, bgcolor: theme.palette.grey[300] }}
        >
          {error}
        </Alert>
      </Box>
              ...
              <CardContent>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <Typography variant="body1">
                      {reading.averageConsumption != null
                        ? `${reading.averageConsumption} m³`
                        : "N/A"}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    ...
                    <Typography variant="body1">
                      {reading.period
                        ? format(new Date(reading.period), "MMM yyyy", {
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Chip
                        ...
                        sx={{
                          bgcolor: reading.reviewed
                            ? theme.palette.greenAccent.main
                            : theme.palette.grey[300],
                          color: reading.reviewed
                            ? "#fff"
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Chip
                        ...
                        sx={{
                          bgcolor: reading.resolved
                            ? theme.palette.greenAccent.main
                            : theme.palette.grey[300],
                          color: reading.resolved
                            ? "#fff"
                            : theme.palette.grey[100],
                        }}
                        size="small"
