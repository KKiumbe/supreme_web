...
const CreateCustomerScreen = () => {
  ...
  const fetchTariffCategories = async () => {
    try {
      const res = await axios.get(`${API_URL}/tarrifs/block`, {
        withCredentials: true,
      });
      ...
      // Silently handle 403 permission errors
      if (err.response?.status === 403) {
        console.warn("Access denied: missing permission for tariffs");
      } else {
  ...
  const handleCreateCustomer = async () => {
    ...
    if (!form.tariffCategoryId) {
      return setError("Tariff Category is required");
    }
    ...
    const payload = {
      ...
      customerDeposit: form.customerDeposit
        ? Number(form.customerDeposit)
        : null,
  ...
  return (
    <Box sx={{ height: "100vh", p: 3 }}>
      {permissionDenied ? (
        <PermissionDeniedUI permission={["customers:view", "surveys:view"]} />
      ) : (
        <>
            ...
            <DialogContent>
              <Stack spacing={2} mt={2}>
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, customerIdNo: e.target.value }))
                  }
                />
                <TextField
                  label="Deposit"
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, customerDeposit: e.target.value }))
                  }
                />
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, tariffCategoryId: e.target.value }))
                  }
          ...
          >
            <DialogTitle>
              Upload Documents — Customer ID: {createdCustomerId}
            </DialogTitle>
            <DialogContent>
