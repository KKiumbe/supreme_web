...
const CustomerEditScreen = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const normalizedData = {
          ...
          closingBalance:
            fetchedData.closingBalance !== null &&
            fetchedData.closingBalance !== undefined
  ...
  const handleLeaseUpload = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message:
          error.response?.data?.message || "Failed to upload lease agreement.",
        severity: "error",
      });
    } finally {
      setSending(false);
  ...
  const handleLeaseDownload = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `lease_${customerData.firstName}_${customerData.lastName}.pdf`,
      );
      ...
      setSnackbar({
        ...
        message:
          ...
          "Failed to download lease agreement.",
        severity: "error",
      });
    } finally {
      setSending(false);
  ...
  return (
    <Container sx={{ maxWidth: 900, minWidth: 600, ml: 10 }}>
      {permissionDenied ? (
        <PermissionDeniedUI permission="customers:edit" />
      ) : (
        <>
          <Paper elevation={3} sx={{ p: 4, mt: 4, minWidth: 800 }}>
            <Typography variant="h4" gutterBottom>
              <TitleComponent
                title={`Edit ${customerData?.firstName}'s Details`}
              />
            </Typography>
              ...
              {!customerData.leaseFileUrl ? (
                <>
                  <Typography variant="body1" mb={2}>
                    No lease agreement found. Upload a new lease agreement (PDF
                    only, max 5MB).
                  </Typography>
                ...
                <>
                  <Typography variant="body1" mb={2}>
                    Lease agreement exists. You can download or terminate the
                    lease.
                  </Typography>
                  <Stack direction="row" spacing={2}>
                    <Button
